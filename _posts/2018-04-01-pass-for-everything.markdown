...
---
&nbsp;&nbsp;&nbsp;&nbsp;Password management is a pain. If you're keeping all your passwords in your head, chances are you have to keep 10-12 stuffed up there at a given time.
 That's if you're not recycling passwords across multiple services (if you are, shame on you).
 Keeping many more than that in your head at a given time is an absolute nightmare.
...
&nbsp;&nbsp;&nbsp;&nbsp;Imagine this, tomorrow morning you wake up and get an email from your bank.
 ...
 Later that day you go to log in to your student loan management site.
 "Wait, which password was that that again?" You ask yourself, and you begin shuffling through every password you can remember and still come up blank.
 You go reset your password after about 5 minutes of wasted time (assuming you didn't lock your account) and then have to add another password to that leaky bucket you call a brain.

&nbsp;&nbsp;&nbsp;&nbsp;Thankfully, we have password managers for us these days.
 ...
 There's all kinds of software out there to manage this for us.
 You can use `keepass`, `LastPass`, `Intel True Key`, or my weapon of choice, `pass`.
...
&nbsp;&nbsp;&nbsp;&nbsp;This article is not a comparison of all these different password managers.
 ...
 This I find to be actually useful because getting this to work (particularly across multiple systems) is tricky as there are a lot of places where you can get stuck for easily a few hours if you're new to some of this software.
...
&nbsp;&nbsp;&nbsp;&nbsp;Now you're going to generate your key pair. For the sake of this example we're going to assume that you don't want the key to expire and you implicitly trust everyone and all the devices that you're going to give this key to.
 That's probably not the safest, but I'll save the the concept of securing your gpg keys for another time.
...
#### A Small Concern ####
&nbsp;&nbsp;&nbsp;&nbsp;However there is one fairly common concern that I think is best addressed now rather than later.
 There are concerns about pass being "leaky" when it comes to user and service metadata.
 That's not completely true.
 If you do something like the above example where I showed you `pass insert service1/user1` if somehow someone gets a hold of your raw store they now know you have an account (user1) at a specific service (service1).
 ...
 Though this may be a secondary concern, this is really only useful if they can decrypt them with your private key, which should be password protected as well.
 This isn't the most likely point of failure, but it's more information than the hacker may have had before.
...
&nbsp;&nbsp;&nbsp;&nbsp;You can use pass all over the place, so long as you can decrypt the files.
 For the sake of this example I'm just going to use a one computer, one Android phone setup.
 You can scale this pretty easily across however many devices you have.
 One more thing though, this is what I'm going to call "the dirty way" of doing this.
 The safest way is probably to have a gpg key for each device, then to trust both of these keys.
 Or, use some sort of hardware key, like yubikey for decrpyting these on you different devices.
...
&nbsp;&nbsp;&nbsp;&nbsp;I suggest creating the the repo from whatever online client first, if you can, then clone that and start the password store management business.
 Reason being, the android app were going to be using only supports bare repositories.
...
&nbsp;&nbsp;&nbsp;&nbsp;Getting on with it you're going to need a few things.
 First, a remote git repo in which to store your passwords.
...
&nbsp;&nbsp;&nbsp;&nbsp;Once you get those apps open Password Store first open up settings and make sure external repository is disabled.
 If you're not already you should interacting with remote via SSH.
...
&nbsp;&nbsp;&nbsp;&nbsp;Now is for the dicey part, getting your secret key over to your android device.
...
```
 ...
 DO NOT, FOR THE LOVE OF OLD PAPA LINUS, SEND YOUR KEY TO YOURSELF IN AN UNECRYTPED EMAIL!

&nbsp;&nbsp;&nbsp;&nbsp;Once again, your private key is the one thing above all else that you do not want compromised.
...
There are plenty of common sense methods you can do to keep it secure with trusted keys and yubikey that I will go into at a later date.
